...
test("groups sessions into calendar-aware recency buckets", () => {
  // Dates are built in the local timezone so the assertions are stable no
  // matter where the suite runs (the implementation buckets by local day).
  const now = new Date(2026, 6, 15, 18, 0, 0); // Jul 15 18:00 local
