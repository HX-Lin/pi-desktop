import type { Message } from "../../vendor/jev/index";
...
export function renderTranscript(messages: readonly Message[]): string {
  ...
  for (const message of messages) {
    ...
    if (message.toolUses.length > 0) {
      lines.push(
        `[Assistant tool calls]: ${message.toolUses.map((call) => renderCall(call.tool, call.input)).join("; ")}`,
      );
    }
...
export function renderSummary(messages: readonly Message[], options: SummaryOptions = {}): string {
  ...
  const header = ["Earlier conversation retained by Jev selective compaction.", "User and assistant text is verbatim."];
  ...
  if ((options.truncatedResults ?? 0) > 0) {
    header.push(
      `${options.truncatedResults} tool results were truncated (marked); re-run a tool if its full output is needed again.`,
    );
  }
